...
import Ajv from "ajv";
let { output, multiline, name, module, deref, 'resolve-circulars': resolveCirculars, defaults, 'exit-on-error': exitOnError } = parser()
    ...
    multiline: {
        ...
        alias: "ml",
        description: "enable multiline: continues parsing, where each input/output schema is seperated with \\n\\n, only works with --output schema",
    },
    name: {
        type: z
            .string()
    ...
    'resolve-circulars': {
        ...
        alias: "rc",
        description: "Whether to resolve circular references while dereferencing",
    },
    defaults: {
        type: z.boolean().default(true),
        alias: "d",
        description: "use default values in the schema, only works with --output module",
    },
    'exit-on-error': {
        type: z.boolean().default(true),
...
const eprintln = (val = "") => process.stderr.write('ERR: ' + val + "\n");
const println = (val = "") => process.stdout.write(val + "\n");
if (multiline && output !== "schema") {
    eprintln("WARN: multiline flag not working with --output module, ignoring it");
    multiline = false;
}
...
async function* readEach(stream) {
